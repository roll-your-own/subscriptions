...
import { centsToCurrency, nextBillingDate, pluralizeInterval } from '../../utils';
...
export const Plan = ({ dbUser }) => {
  ...
  return (
    <div className="plan" data-testid="route-plan">
      ...
      <h2>{plan.name}</h2>
      <h3 style={{ paddingTop: 0}}>{centsToCurrency(plan.amount)} {plan.currency} every {plan.intervalCount > 1 ? plan.intervalCount : null } {pluralizeInterval(plan.interval, plan.intervalCount)}</h3>
